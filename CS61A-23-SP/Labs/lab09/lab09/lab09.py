...
def is_bst(t):
    ...
    False
    """
    def bst_min(t):      # right's min > label 
        if t.is_leaf():
            return t.label
        return min(t.label, bst_min(t.branches[0]))

    def bst_max(t):      # left's max <= label 
        if t.is_leaf():
            return t.label
    ...
    if len(t.branches) == 1:
        ...
        c_l, c_r = t.branches
        return is_bst(c_l) and is_bst(c_r) and bst_max(c_l) <= t.label and bst_min(c_r) > t.label
    else:
        return False
