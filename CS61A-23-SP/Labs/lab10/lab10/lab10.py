...
def insert_into_all(item, nested_list):
    ...
    [[0], [0, 1, 2], [0, 3]]
    """
    new_nested_list = [[item] + elem for elem in nested_list]
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n == 1:
        ...
        return sum(num_trees(n-k) * num_trees(k) for k in range(n) if k != 0)
...
def insert(link, value, index):
    ...
    if index == 0:
        ...
        while index:
            ...
            link = link.rest
            index -= 1
        save = Link(link.first,link.rest)
        link.first = value 
        link.rest = save
...
def reverse_other(t):
    ...
    def helper(t, reverse):
        if t.is_leaf():
            ...
            reversed = [b.label for b in t.branches][::-1]
            for index in range(len(t.branches)):
                helper(t.branches[index], not reverse)
                if reverse:
                    t.branches[index].label = reversed[index]
