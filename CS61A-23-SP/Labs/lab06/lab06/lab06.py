...
def insert_items(lst, entry, elem):
    ...
    for index in range(len(lst)):
        if lst[index+add_num]==entry:
            lst.insert(index+add_num+1,elem)
            add_num+=1
    return lst        
...
def partial_reverse(lst, start):
    ...
    """
    index=start
    while index<=(len(lst)-1+start)//2:
        lst[index],lst[len(lst)-1+start-index]=lst[len(lst)-1+start-index],lst[index]
        index+=1
