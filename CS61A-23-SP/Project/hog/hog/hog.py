...
def is_always_roll(strategy, goal=GOAL):
    ...
    time0=0
    time1=0
    last=strategy(time0,time1)
    while time0<goal:
        time1+=1
        next=strategy(time0,time1)
        if last!=next:
            return False  
        if(time1==goal):
            time0+=1
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    i=2
    max_i=1
    max=averaged_roll_dice(1,dice)
    while i<=10:
        if max<averaged_roll_dice(i,dice):
...
def final_strategy(score, opponent_score):
    ...
    >>> dice=make_test_dice(2,2,3,3,4,4)
    >>> final_strategy(59, 75)
    6
    >>> dice=make_test_dice(1,1,4,5,1,4)
    >>> final_strategy(59, 75)
    0
    """
    # BEGIN PROBLEM 12
    if take_turn(6, opponent_score, dice=six_sided)>take_turn(0, opponent_score, dice=six_sided):
