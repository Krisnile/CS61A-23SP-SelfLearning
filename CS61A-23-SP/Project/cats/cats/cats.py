...
def pick(paragraphs, select, k):
    ...
    for elem in paragraphs:
        if select(elem):
            pick_select+=[elem]
    if k<len(pick_select):        
        return pick_select[k]
    else:
        return ''        
    # END PROBLEM 1
...
def about(subject):
    ...
    def helper(paragraph):
        for elem_sub in subject:
            for elem_para in split(remove_punctuation(paragraph)):
                if lower(elem_sub)==lower(elem_para):
                    return True
        return False
    return helper
    # END PROBLEM 2
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    min,record=diff_function(typed_word,word_list[0],limit),word_list[0]
    for elem in word_list:
        if elem==typed_word:
            return typed_word  
        if min>diff_function(typed_word,elem,limit):
            min,record=diff_function(typed_word,elem,limit),elem
    if min>limit:
        return typed_word
    else:
        return record  
...
def final_diff(typed, source, limit):
    ...
    elif len(typed)>=2 and len(source)>=2 and typed[0]==source[1] and source[0]==typed[1] :
        return final_diff(typed[2:],source[2:],limit-1)+1
    elif len(typed)>=2 and len(source)>=2 and source[0]==source[1]==typed[0]!=typed[1]:
...
def fastest_words(match):
    ...
    for word_index in word_indices:
        player,min=0,time(match, 0, word_index)
        for player_index in player_indices:
            if time(match, player_index, word_index)<min or time(match, player_index, word_index)==min and player_index<player:
                min=time(match, player_index, word_index)
                player=player_index
        fast[player].append(get_word(match, word_index))
    return fast    
    # END PROBLEM 10
